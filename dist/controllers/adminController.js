...
const crearTurnoReprogramado = async (req, res) => {
    ...
    `;
        ...
        const guiaQuery = await connection_1.pool.query('SELECT id, nombre, disponible FROM guias WHERE id = $1', [guiaId]);
...
const asignarGuiaATurno = async (req, res) => {
    try {
        ...
        const guiaQuery = await connection_1.pool.query('SELECT id, nombre FROM guias WHERE id = $1', [guiaId]);
        if (guiaQuery.rows.length === 0) {
            res.status(404).json({ error: 'Guía no encontrado' });
            return;
        }
...
const getTodosUsuariosConGuia = async (req, res) => {
    try {
      ...
      LEFT JOIN guias g ON ultimo.guia_id = g.id
